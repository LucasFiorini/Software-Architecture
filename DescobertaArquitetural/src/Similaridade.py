...
#            print(ast.dump(self.arvore))
    ...
    def filtrar_metodos(self):
        for classe in self.lista_info_classes:
            lista_filtrada = []
            self.map_classe_classe[classe.name] = []
            for metodo in classe.metodos_externos:
                if self.eh_externo(metodo, classe.name):
                    nome_classe_externa = self.encontra_nome_classe(metodo)
                    if nome_classe_externa not in self.map_classe_classe[classe.name]:
                        self.map_classe_classe[classe.name].append(nome_classe_externa)
    ...
    def encontra_nome_classe(self, nome_metodo):
        for classe in self.map_classe_metodos.keys():
            if nome_metodo in self.map_classe_metodos[classe]:
                return classe
